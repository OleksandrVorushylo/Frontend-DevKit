...
export function rgbaToHex({ r, g, b, a = 1 }) {
  const toHex = (val) => clampNumber(Math.round(val), 0, 255).toString(16).padStart(2, '0');
  const alpha = typeof a === 'number' ? toHex(Math.round(clampNumber(a, 0, 1) * 255)) : '';
...
export function rgbaString({ r, g, b, a }) {
  const alpha = typeof a === 'number' ? Math.round(a * 1000) / 1000 : 1;
...
export function parseRgbInput(input) {
  if (!input) return null;
